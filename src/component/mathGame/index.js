...
import Navbar from '../mathNavbar'

import GameItem from '../mathGameItem'

import TabItem from '../mathItemTab'
...
class MathGame extends Component {
  state = {
    ...
    searchInput: '',
    timer: 60,
    bgImage: imagesList[0].imageUrl,
    bgIds: imagesList[0].id,
    Score: 0,
  }
  ...
  getFiltereddApps = () => {
    ...
    const FilteredAppsList = imagesList.filter(
  ...
  changeImage = id => {
    const {bgIds, Score} = this.state
    const number = imagesList.length
    const RandomNumber = Math.ceil(Math.random() * number)
    if (id === bgIds) {
      this.setState(prevState => ({
        Score: prevState.Score + 1,
        bgIds: imagesList[RandomNumber].id,
        bgImage: imagesList[RandomNumber].imageUrl,
      }))
    } else {
      this.setState({
        bgIds: imagesList[RandomNumber].id,
  ...
  render() {
    const {timer, TABID, bgImage, Score} = this.state
    const FilteredApps = this.getFiltereddApps()
    let TimerVariable
    if (timer > 0) {
      TimerVariable = true
    } else {
        ...
        {TimerVariable ? (
          <div>
            ...
            <div className="main-bg">
              <img src={bgImage} className="main-image" alt="match" />
              <div>
                <ul className="tab-items">
                  {tabsList.map(EachCard => (
                    <TabItem
                      EachCard={EachCard}
                      clickTabItem={this.clickTabItem}
                      onSelectedChange={EachCard.tabId === TABID}
                      key={EachCard.tabId}
                    />
                  ))}
                </ul>
          ...
          <div className="second-bg">
            <div className="gameover-bg">
              <img
                ...
                className="image-trophy"
                alt="trophy"
              />
              <p className="scores">Your Score</p>
